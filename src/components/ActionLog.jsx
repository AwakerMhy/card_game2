export default function ActionLog({ entries = [], maxEntries = 16, mobileLayout = false, mobileLarge = false }) {
  ...
  const mobileMaxH = mobileLarge ? "max-h-40" : "max-h-24";
  ...
  return (
    ...
    >
      <div className={`p-2 font-bold text-amber-400 border-b border-slate-600 sticky top-0 bg-slate-800 ${mobileLayout && mobileLarge ? "text-base" : "text-xs"}`}>
        操作记录
      </div>
      <div className="p-2 space-y-1 text-sm">
        {display.length === 0 ? (
          <div className={`text-slate-500 py-2 ${mobileLayout && mobileLarge ? "text-base" : "text-xs"}`}>暂无记录</div>
        ) : (
          display.map((entry, i) => (
            <div
              key={i}
              className={`py-1 px-2 rounded ${mobileLayout && mobileLarge ? "text-base" : "text-xs"} ${
